...
static const NSString * CSToastBlockKey         = @"CSToastBlockKey";
...
- (UIView*)makeToast:(NSString *)message {
    return [self makeToast:message duration:CSToastDefaultDuration position:CSToastDefaultPosition];
}
...
- (void)setTapDismissBlock:(void(^)(UIView* toast))block {
  objc_setAssociatedObject(self, &CSToastBlockKey, block, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)handleToastTapped:(UITapGestureRecognizer *)recognizer {
    ...
    [timer invalidate];
    void (^block)(UIView*) = objc_getAssociatedObject(self, &CSToastBlockKey);
    if (block) { block(self); }
